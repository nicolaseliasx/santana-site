...
function dt_theme_public_title() {
	...
	if (!$status) :
		...
		// is_search()
		elseif (is_search ()) :
			$args ["search"] = __ ( "Resultados encontrados para", 'iamd_text_domain' ) . ' "' . $_REQUEST ['s'] . '"'; // dding search text into the default args
		...
		elseif (is_404 ()) :
			...
			foreach ( $options as $option ) :
				if (array_key_exists ( $option, $args ))
					$doctitle .= $args [$option] . ' ' . $separator . ' ';
			endforeach;
			$doctitle = $doctitle . __ ( 'Página não encontrada', 'iamd_text_domain' );
			$split = false;
		...
		elseif( ('tribe_events'== get_post_type() || is_singular('tribe_events') || is_singular('tribe_venue') || is_singular('tribe_organizer') || in_array('tribe-filter-live', get_body_class())) and !is_search()):			
			$doctitle = $args ["blog_title"] . ' ' . $separator . ' ' . __ ( 'Eventos', 'iamd_text_domain' );
			$split = false;
		endif;
