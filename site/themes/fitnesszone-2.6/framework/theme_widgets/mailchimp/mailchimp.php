...
function dt_theme_mailchimp_subscribe() {
	...
	if($apiKey != '' && $listId != '') {
		...
		if(dt_theme_mailchimp_check_member_already_registered($data, $apiKey, $listId)) {
			$out = '<span class="error-msg"><b>'.esc_html__('Erro:', 'iamd_text_domain').'</b> '.esc_html__('You have already subscribed with us !', 'iamd_text_domain').'</span>';
		} else {
			$out = dt_theme_mailchimp_register_member($data, $apiKey, $listId);
		}
				
	} else {
		$out = '<span class="error-msg"><b>'.esc_html__('Erro:', 'iamd_text_domain').'</b> '.esc_html__('Please make sure valid mailchimp details are provided.', 'iamd_text_domain').'</span>';
	}
...
function dt_theme_mailchimp_register_member($data, $apiKey, $listId) {
	...
	if($httpCode == 200) {
		$out = '<span class="success-msg">'.esc_html__('Successo! Por favor verifique a sua caixa de email ou spam.', 'iamd_text_domain').'</span>';
	} else {
