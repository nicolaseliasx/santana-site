...
class MY_Mailchimp extends WP_Widget {
	...
	function form($instance) {
		...
		if( dt_theme_option('general','mailchimp-key') ):
            ...
            <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Título:','iamd_text_domain');?> 
               <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>"  
                      type="text" value="<?php echo esc_attr($title); ?>" /></label></p>
                      
            <p><label for="<?php echo $this->get_field_id('list_id'); ?>"><?php _e('Selecionar lista:','iamd_text_domain'); ?></label>
...
<?php   else:
	...
	function widget($args, $instance) {
		...
		echo '<p>'.__('Assine nossa Newsletter e acompanhe nossos conteúdos e novidades.', 'iamd_text_domain').'</p>';

		echo '<form name="frmsubscribe" method="post" class="subscribe-frm">';
		echo '<input type="email" required name="dt_mc_emailid" id="dt_mc_emailid" placeholder="'.__('Digite seu e-mail...', 'iamd_text_domain').'" />';
		...
		echo '<input type="submit" name="submit" class="dt-sc-button small" value="'.__('Assinar', 'iamd_text_domain').'" />';
		echo '</form>';
