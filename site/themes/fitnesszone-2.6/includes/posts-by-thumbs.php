<?php
	...
	if($the_query->have_posts()): $i = 1;
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('blog-entry'); ?>>
				<div class="blog-entry-inner">
					...
					<div class="entry-metadata"><?php
                    	if(is_sticky()): ?>
                          <div class="featured-post"><span><?php _e('Destaque','iamd_text_domain'); ?></span></div><div class="clear"></div>
						...
						<a href="<?php the_permalink(); ?>"><?php _e('Leia mais', 'iamd_text_domain'); ?> <i class="fa fa-angle-double-right"></i></a>
					</div>
				</div>
			</article>
		</div><?php
	 ...
	 wp_reset_postdata();
		...
		<h2><?php _e('Não encontrado.', 'iamd_text_domain'); ?></h2>
		<p><?php _e('Desculpas, mas nenhum resultado foi encontrado para o arquivo solicitado.', 'iamd_text_domain'); ?></p><?php
    endif; ?>