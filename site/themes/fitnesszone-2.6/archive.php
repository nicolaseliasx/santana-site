<?php get_header();
      ...
      <div class="breadcrumb-wrapper">
          <div class="container">
              <h1><?php if(is_category()): _e('Categoria de arquivos: ', 'iamd_text_domain'); echo single_cat_title();
                          elseif(is_day()):  _e('Arquivos diários de: ', 'iamd_text_domain'); echo get_the_date('l');
                          elseif(is_month()):  _e('Arquivos mensais de: ', 'iamd_text_domain'); echo get_the_date('F, Y');
                          elseif(is_year()):  _e('Arquivos anuais de: ', 'iamd_text_domain'); echo get_the_date('Y');
                          elseif(is_author()):  _e('Arquivo por Autor: ', 'iamd_text_domain'); $author = get_user_by('login', get_query_var('author_name')); echo $author->nickname;
                          elseif(is_tag()):  _e('Marcar Arquivos de: ', 'iamd_text_domain'); echo single_tag_title('', true);
                          elseif(taxonomy_exists('category')):  _e('Arquivo de: ', 'iamd_text_domain'); echo single_cat_title();
                          endif; ?></h1>
