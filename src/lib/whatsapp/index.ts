...
export function productWhatsApp(product: { name: string; code?: string; slug: string; priceFormatted?: string }, origin = 'https://santanafitness.com.br') {
  ...
  const priceLine = product.priceFormatted ? `\nValor: ${product.priceFormatted}` : '';
  return whatsappUrl(`Olá! Tenho interesse no ${interest}.\nGostaria de mais informações.\n\nProduto:\n${safeOrigin}/produtos/${product.slug}/${priceLine}`);
}
