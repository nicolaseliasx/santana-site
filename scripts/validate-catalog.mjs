...
for (const product of products) {
  ...
  if (product.price != null && (!(product.price > 0) || product.priceFormatted !== expectedPrice(product.price))) errors.push(`invalid price: ${product.slug}`);
  if (!Array.isArray(product.categorySlugs) || product.categorySlugs.length !== 1) errors.push(`missing imported fields: ${product.slug}`);
...
const expectedContentSlugs = new Set([...expectedSlugs, ...products.map((product) => product.slug)]);
for (const slug of expectedContentSlugs) if (!contentSlugs.has(slug)) errors.push(`missing product content: ${slug}`);
for (const slug of contentSlugs) if (!expectedContentSlugs.has(slug)) errors.push(`stale product content: ${slug}`);
...
if (products.length !== 247 || productFiles.length !== 247 || report.categorylessCount || report.uncategorizedCount || errors.length) process.exit(1);
